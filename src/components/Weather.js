...
const Weather = ({ city }) => {
  ...
  // NOTE: openweather results for 5 day forecast
  const dailyForecast = data?.city && data.list.filter((element, item) => item % 8 === 8 - 1)
  ...
  const desc = data?.list[0]?.weather[0]?.description && data.list[0].weather[0].description;
  const icon = data?.list[0]?.weather[0]?.icon && data.list[0].weather[0].icon;
  const day = displayDay(data?.list[0]?.dt_txt);
  
  return (
    <StyledWeather>
      {data === null && <>Search by city or country to get the weather</>}
      <div className="layout">
      <div className="today">
          <div className="location">
            {name && <p className="city">{name}</p>}
            {name && <p className="country">{country}</p>}
            {name && <p className="date">{date}</p>}
          </div>
          <div className="weather">
            {temp && <p className="temp">{temp}<sup>°c</sup></p>}
            {desc && <img className="icon" alt={desc} src={`${iconURL}${icon}@4x.png`} />}
